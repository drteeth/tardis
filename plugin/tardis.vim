" Adapted from https://raw.githubusercontent.com/tardis/vim-tardis/master/plugin/tardis.vim
...
" Init {{{
    ...
    if exists("g:loaded_tardis")
        finish
    endif
    let g:loaded_tardis = s:true
    ...
    " Script Globals
    let s:home = expand("$TARDIS_HOME")
    if s:home == '$TARDIS_HOME'
        let s:home = expand("$HOME")
    endif
    let s:cli_location = substitute(substitute(expand("<sfile>:p:h"), '\', '/', 'g'), '/plugin$', '', '') . '/target/debug/tardis'
    let s:config_file = s:home . '/.tardis.cfg'
    ...
    let s:data_file = s:home . '/.tardis.data'
    ...
    function! s:Init()
        ...
        if !exists("g:tardis_HeartbeatFrequency")
            let g:tardis_HeartbeatFrequency = 2
        endif

        " Get legacy g:tardis_ScreenRedraw setting
        let s:redraw_setting = 'auto'
        if exists("g:tardis_ScreenRedraw") && g:tardis_ScreenRedraw
            let s:redraw_setting = 'enabled'
        endif

        " Get redraw setting from tardis.cfg file
...
" Function Definitions {{{
    ...
    function! s:SendHeartbeats()
        ...
        let cmd = cmd + ['--branch', branch]
        let cmd = cmd + ['--time', heartbeat.time]
        let cmd = cmd + ['--plugin', printf('vim/%s vim-tardis/%s', s:n2s(v:version), s:VERSION)]
        ...
        if s:is_debug_on && stdout != ''
            echoerr '[Tardis] Heartbeat Command: ' . s:JoinArgs(cmd) . "\n[Tardis] Error: " . stdout
        endif
    endfunction
    ...
    function! s:AsyncHandler(output, cmd)
        if s:is_debug_on && a:output != ''
            echoerr '[Tardis] Heartbeat Command: ' . s:JoinArgs(a:cmd) . "\n[Tardis] Error: " . a:output
        endif
    endfunction
    ...
    function! s:NeovimAsyncExitHandler(job_id, exit_code, event)
        ...
        if (s:is_debug_on || a:exit_code == 103 || a:exit_code == 104) && (a:exit_code != 0 || output != '')
            echoerr printf('[Tardis] Error %d: %s', a:exit_code, output)
        endif
    endfunction
    ...
    function! s:EnoughTimePassed(now, last)
        ...
        if a:now - prev > g:tardis_HeartbeatFrequency * 60
            return s:true
        endif
        return s:false
    endfunction
...
" Plugin Commands {{{
    ...
    :command -nargs=0 TardisDebugEnable call s:EnableDebugMode()
    :command -nargs=0 TardisDebugDisable call s:DisableDebugMode()
    :command -nargs=0 TardisScreenRedrawDisable call s:DisableScreenRedraw()
    :command -nargs=0 TardisScreenRedrawEnable call s:EnableScreenRedraw()
    :command -nargs=0 TardisScreenRedrawEnableAuto call s:EnableScreenRedrawAuto()
    :command -nargs=0 TardisToday call g:TardisToday()
